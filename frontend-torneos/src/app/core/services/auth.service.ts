...
interface DevCredentials { email: string; password: string; }
...
export class AuthService {
  ...
  devLogin(email: string, password: string) {
    return this.http
      ...
      .pipe(tap(({ token }) => { this.saveToken(token); this.fetchMe(); }));
  }
  ...
  fetchMe(): void {
    this.http.get<AuthUser>(`${environment.apiUrl}/auth/me`).subscribe({
      next: (user) => {
        ...
        if (returnUrl) { localStorage.removeItem('sc_return_url'); this.router.navigateByUrl(returnUrl); return; }
  ...
  updateUser(user: AuthUser): void { this.saveUser(user); }
  ...
  private saveToken(token: string): void { localStorage.setItem(TOKEN_KEY, token); this._token.set(token); }
  private saveUser(user: AuthUser): void  { localStorage.setItem(USER_KEY, JSON.stringify(user)); this._user.set(user); }
